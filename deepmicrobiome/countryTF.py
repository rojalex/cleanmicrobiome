...
full = full.drop('City', axis=1)
full = full.drop('BodySite', axis=1)
full_X = full.drop('Country', axis=1)
X = full_X.values
full_y = full['Country']
y = full_y.values
...
integer_encoded_train = label_encoder.fit_transform(y_train) #puts into integers up to 34 for train
...
y1_train = onehot_encoder.fit_transform(integer_encoded_train)
...
def train_test_model(hparams):
    ...
    model.add(Dropout(hparams[HP_DROPOUT]))
    model.add(Dense(units=35, activation='softmax')) #change based on data c/s/c
...
best_model.save('countrymodel.h5')
import joblib
joblib.dump(label_encoder, 'countrylabel.pkl')
joblib.dump(onehot_encoder, 'countryonehot.pkl')

#output file
f = open('countryout.txt', 'w')
f.write('score:\n')
f.write(str(score))
f.write('matrix:\n')
f.write(str(matrix))
