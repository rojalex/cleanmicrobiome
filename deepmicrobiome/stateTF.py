...
def train_test_model(hparams):
    ...
    model.add(Dropout(hparams[HP_DROPOUT]))
    model.add(Dense(units=91, activation='softmax')) #change based on data c/s/c
...
best_model.save('statemodel.h5')
import joblib
joblib.dump(label_encoder, 'statelabel.pkl')
joblib.dump(onehot_encoder, 'stateonehot.pkl')

#output file
f = open('stateout.txt', 'w')
f.write('score:\n')
f.write(str(score))
f.write('matrix:\n')
f.write(str(matrix))
